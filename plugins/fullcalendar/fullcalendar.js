  ...
  function fetchResourceSource(source, currentView) {
    ...
    if (resources) {
      if ($.isFunction(resources)) {
        return resources(function(data){return data});
      }
    } else {
      var url = source.url;
      if (url) {
...
function ResourceView(element, calendar, viewName) {
  ...
  // exports
	...
	function buildSkeleton() {
		...
		slotLayer =
			$("<div class='custom_div' style='position:absolute;z-index:2;left:0;width:100%'/>")
				.appendTo(element);
	...
	function setHeight(height) {
		...
		slotHeight = (slotHeight0 + slotHeight1) / 2;
                ...
                slotLayer.css('top',(slotHeight0 + slotHeight1));
